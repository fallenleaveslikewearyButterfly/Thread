import requests, time,json,queue,threading
...
class proxycrawler():
    ...
    def dispath(self):
        urls=queue.Queue()
        for i in range(1,16):
            urls.put("http://www.kuaidaili.com/free/inha/" + str(i) + "/")
        #开启三个线程抓代理
        self.producepool.submit(self.xunproxy,)
        futures={self.producepool.submit(self.kuaiproxy, urls): x for x in range(0,3)}
        for future in futures:
            while not future.done():
                pass
        print(self.proxy.qsize())
        futuresval={self.consumerpool.submit(self.validate,):x for x in range(0,15)}
        for future in futuresval:
            while not future.done():
                pass
        return self.proxyok
